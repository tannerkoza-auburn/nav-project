...
r_eb_n(2,:)=[0 0 0];
C_b_n=eye(3);
Lb=data.truth.LLA(1,1);
lamb=data.truth.LLA(1,2);
...
omega_en_n=[v_eb_n(t-1,2)/(Re*Lb+hb) -v_eb_n(t-1,1)/(Re*Lb+hb)...
    (-v_eb_n(t-1,2)*tan(Lb))/(Re*Lb+hb)];
...
%acceleration
    acc_eb_n(t,:)=f_ib_n-(omegak_en_n+2*omegak_ie_n)*v_eb_n(t-1,:)';
%velocity
    v_eb_n(t,:)=(v_eb_n(t-1,:)'+acc_eb_n(t-1,:)'*dt)';

Lb(t)=Lb(t-1)+dt*(v_eb_n(t-1,1)/(Rn+hb));
lamb(t)=lamb(t-1)+dt*(v_eb_n(t-1,2)/(cos(Lb)*(Re+hb)));
hb(t)=hb(t-1)-dt*v_eb_n(t-1,3);
end
